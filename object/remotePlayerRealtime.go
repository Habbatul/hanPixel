...
type RemotePlayerRealtime struct {
	x, y         float64
	frameIndex   int
	image        *ebiten.Image
	lastX, lastY float64
...
func (rp *RemotePlayerRealtime) UpdateAnimation(newX, newY float64) {
	const frameCount = 6
...
func NewRemotePlayerRealtime(x, y float64) *RemotePlayerRealtime {
	img, _, err := ebitenutil.NewImageFromFile("game_asset/asset_sprite/player/Unarmed_Walk_full.png")
	if err != nil {
		log.Println("RemotePlayerRealtime image load error:", err)
		img = ebiten.NewImage(64, 64) // fallback blank
	}
	return &RemotePlayerRealtime{
		x:     x,
		y:     y,
		image: img,
	}
...
func (rp *RemotePlayerRealtime) Draw(screen *ebiten.Image, camera *Camera) {
	const (
		frameWidth   = 64
		frameHeight  = 64
		framesPerRow = 6
...
func (rp *RemotePlayerRealtime) GetY() float64 {
	return rp.y
}

func (rp *RemotePlayerRealtime) SetX(x float64) {
	rp.lastX = rp.x
	rp.x = x
}

func (rp *RemotePlayerRealtime) SetY(y float64) {
	rp.lastY = rp.y
	rp.y = y
}
